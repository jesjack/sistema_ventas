...
class SheetAdmin:
    ...
    def _create_unlocked_cell_protection(self):
        import uno

        protection = uno.createUnoStruct("com.sun.star.util.CellProtection")
        protection.IsLocked = False
        protection.IsHidden = False
        protection.IsFormulaHidden = False
        protection.IsPrintHidden = False
        return protection

    @contextmanager
    def temporary_unlock(self, password=""):
        try:
            self.hoja.unprotect(password)
            self.hoja.getCellByPosition(1, 3).CellProtection = self._create_unlocked_cell_protection()
            self.hoja.getCellByPosition(2, 3).CellProtection = self._create_unlocked_cell_protection()
            self.hoja.getCellByPosition(3, 3).CellProtection = self._create_unlocked_cell_protection()
            yield
        finally:
            self.hoja.protect(password)
    ...
    def _get_time_format_key(self, document=None, locale=None):
        resolved_document = self._resolve_document(document)
        number_formats = resolved_document.getNumberFormats()
        if locale is None:
            import uno
        ...
        format_key = number_formats.queryKey(time_format, resolved_locale, True)
        if format_key == -1:
            format_key = number_formats.addNew(time_format, resolved_locale)
        return format_key

    def _apply_time_format_to_range(self, cell_range, document=None, locale=None):
        cell_range.NumberFormat = self._get_time_format_key(document=document, locale=locale)
        return cell_range.NumberFormat
    ...
    def format_column_as_time(self, index, document=None, locale=None):
        if index < 0:
            raise ValueError("index debe ser mayor o igual que cero")

        max_row_index = 1048575
        columna = self.hoja.getCellRangeByPosition(index, 0, index, max_row_index)
        return self._apply_time_format_to_range(columna, document=document, locale=locale)

    def format_row_as_time(self, index, document=None, locale=None):
        if index < 0:
            raise ValueError("index debe ser mayor o igual que cero")

        max_column_index = 16383
        fila = self.hoja.getCellRangeByPosition(0, index, max_column_index, index)
        return self._apply_time_format_to_range(fila, document=document, locale=locale)
